...
import {cityGuess, addModel, searchplace, foodCategory} from '@/api/getData'
import {baseUrl, baseImgPath} from '@/config/env'
export default {
  data () {
    return {
      formData: {
        models_name: 'sdssdds', // 项目名称
        belong_project: 'belong_module', // 项目负责人
        models_desc: 'sssdasds', // 测试人员
        other_desc: 'dsdasds', // 开发人员
        status: '' // 状态
      },
      rules: {
        name: [
    ...
    submitForm (formName) {
      ...
      this.$refs[formName].validate(async (valid) => {
        ...
        if (valid) {
          ...
          console.log('进入这里')
          try {
            let result = await addModel(this.formData)
            console.log(result)
            if (result.status === 1) {
              this.$message({
                type: 'success',
